...
const TeamWork = ({
	...
	const [data, setData] = useState(data1 !== undefined ? data1 : (
		localStorage.getItem('redwing_data') ? JSON.parse(localStorageData) : {})
	);

	const [projectData, setProjectData] = useState(data1 !== undefined ? data1.projects :
	...
	const sorting = (col, sortingOrder1) => {
		if (col === 'tasks_count' || col === 'active_count' || col === 'weekly_completed_todo' || col === 'weekly_average_per_day') {
					...
					{tabValue === 'Team' && (
						<>
							...
							<div className='flexx'>
								...
								>
									{segmenting === 'Default' ? <p className='dft'>Default</p> : <p className='unactive-segmenting'>Default</p>}
								</p>
								&nbsp;&nbsp;
								<p
